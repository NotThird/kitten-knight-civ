...
- Keeps the "policy management" vibe: you‚Äôre still steering, but now the advisor can be acted on in 1 click instead of manual slider hunting.
...
- Makes Director policy more legible: you can see whether you‚Äôre safe to push growth or should toggle Winter Prep / Crisis.
...
What to try
- Enable **Auto Recruit**, stabilize food/kitten above target in Spring, and watch for the ‚Äústray kitten joined‚Äù log event.
...
What to try
- Load the local build and confirm the game still runs; then open DevTools ‚Üí Network and ensure `css/app.css` + `js/main.js` load (no 404s).
...
## 2026-02-27 13:37 CST ‚Äî v0.8.9 Aptitude bias (skills influence AI + mood)
...
## 2026-02-27 13:52 CST ÔøΩ v0.9.0 Mentoring (science ? skills)

Summary
- Added a new **Mentor** task (unlocks with **Libraries**): spend science to train a lagging kitten in the mentorÔøΩs top skill.
...
Summary
- Added **Autonomy** slider (0ÔøΩ100%) to tune how strongly kittens follow their personality likes/dislikes.
...
What to try
- In Winter, set Work pace to **120%** for ~20‚Äì30s to stabilize warmth/threat, then drop to **90%** and watch mood/efficiency recover.
...
- This makes the colony feel more ìciv simî: same policy can produce different individual behaviors (without turning into pure randomness).
...
What to try
- Set **Autonomy = 80%** and **Mode = Advance**, then watch if different kittens ìfreestyleî into research/industry even while the plan biases food/wood.
...
- Kept Season panelís compact "Projects:" summary line for quick scanning.
...
What to try
- Save a stable setup in **A**, then intentionally wreck your policy (e.g., set Forage=0). Load **A** and watch the plan + assignments snap back within 1ñ2 decision ticks.
